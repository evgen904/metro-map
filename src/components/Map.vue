...
export default {
  ...
  mounted () {
    ...
    if (this.metroMap.$el.querySelector('#scheme-layer-stations')) {
      ...
      this.metroMap.$el.querySelector('#scheme-layer-links').addEventListener('click', (event) => {
        ...
        let selectLink = this.metroMap.findlink(idLink)
        this.metroMap.cloneLink(selectLink)
  ...
  methods: {
    ...
    onScroll (event) {
      ...
      console.log(this.$refs.metroMap.getBoundingClientRect().left - this.$refs.metroMapWr.getBoundingClientRect().left);


      let procX = (event.pageX - this.$refs.metroMapWr.getBoundingClientRect().left) / this.$refs.metroMapWr.clientWidth * 100;
      let procY = (event.pageY - this.$refs.metroMapWr.getBoundingClientRect().top) / this.$refs.metroMapWr.clientHeight * 100;

      this.positionCursor = [event.pageX, event.pageY, ~~procX, ~~procY];
  ...
  watch: {
    ...
    stepZoom (val) {
      ...
      //this.$refs.metroMap.style.transformOrigin = `${this.positionCursor[2]}% ${this.positionCursor[3]}%`;
      //this.$refs.metroMap.style.transform = `translate3d(${this.pos[0]}px, ${this.pos[1]}px, 0px) scale(${scaleVal})`
  ...
  data () {
    return {
      ...
      stepZoom: 0,
      positionMap: [0, 0],
      positionCursor: [null, null, null, null],
      pos: [0, 0],
