<template>
  <div class="map">
    <div class="map--svg" ref="metroMapWr" @mousedown="mapDown($event)">
      <div
        ref="metroMap"
        v-html="svg"
        @wheel.prevent="onScroll($event)"
      ></div>
    </div>
    <div class="map--zoom">
...
export default {
  ...
  methods: {
    ...
    mapDown (event) {
      ...
      this.$refs.metroMap.style.transform = `
        translate3d(
          ${event.offsetX}px,
          ${event.offsetY}px,
          0px)
    ...
    onScroll (event) {
      ...
      this.positionCursor = [posX, posY, event.offsetX, event.offsetY];
  ...
  watch: {
    ...
    stepZoom (val) {
      if (val === 0) {
        this.metroMap.$el.style.transform = `scale(1)`;
      } else if (val === 1) {
        this.metroMap.$el.style.transform = `scale(2)`;
      } else if (val === 2) {
        this.metroMap.$el.style.transform = `scale(4)`;
      }
    }
  },
  computed: {
